...
function buildGitHubHTML(githubConfig) {
  const { username, stats_card, lang_card, snake_dark, pacman, contribution_graph } = githubConfig;
  ...
  return `
    ...
    <!-- Row 4: Pac-Man animation -->
    <div class="gh-pacman-wrap fade-in-hidden" data-delay="350">
      <p class="gh-section-label">Pac-Man Game</p>
      <div class="gh-snake-card">
        <img
          src="${pacman}"
          alt="Pac-Man Contribution Graph ${username}"
          class="gh-snake-img"
          loading="lazy"
          onerror="this.parentElement.innerHTML='<p class=gh-img-fallback>Pac-Man belum tersedia. Pastikan GitHub Actions pacman.yml sudah dijalankan.</p>'"
        />
      </div>
    </div>
